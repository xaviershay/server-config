  ...
  RSpec.describe 'DNS Configuration for rhnh.net' do
    let(:dns) do
      config = {}
...
describe 'MX records' do
  let(:records) { dns.getresources('rhnh.net', Resolv::DNS::Resource::IN::MX) }
...
describe 'TXT records' do
  let(:records) { dns.getresources('rhnh.net', Resolv::DNS::Resource::IN::TXT) }

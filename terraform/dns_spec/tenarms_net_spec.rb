  ...
  RSpec.describe 'DNS Configuration for tenarms.net' do
    let(:dns) do
      config = {}
...
describe 'MX records' do
  let(:records) { dns.getresources('tenarms.net', Resolv::DNS::Resource::IN::MX) }
...
describe 'TXT records' do
  let(:records) { dns.getresources('tenarms.net', Resolv::DNS::Resource::IN::TXT) }
